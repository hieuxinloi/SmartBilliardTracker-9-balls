...
def load_detections(csv_path):
    ...
    frames_data = [
        {"frame_idx": idx, "balls": balls} for idx, balls in sorted(frames.items())
    ]
    return frames_data
...
def merge_sequential_collisions(collisions, max_gap=2):
    ...
    sorted_collisions = sorted(
        unique_collisions, key=lambda x: (x["frame_idx"], x["ball"]["name"])
    )
...
def detect_collision(
    frames_data,
    move_thresh=DEFAULT_MOVE_THRESH,
    ...
    for i, frame_data in enumerate(frames_data):
        ...
        for prev_b in prev_balls:
            ...
            ball_moved = get_moving_balls(
                frames_data, i, prev_b["name"], move_thresh=move_thresh
            )
            if not ball_moved:
                continue
            ...
            if b is not None:
                # Filter: Reject if cueball and ball are overlapping
                # (likely same ball detected twice with different labels)
                center_distance = distance(cueball, b)
                min_separation = (
                    cueball["r"] + b["r"]
                ) * 0.5  # Balls must be at least 50% of combined radii apart

                if center_distance < min_separation:
                    # Balls are overlapping - likely false detection
                    print(
                        f"[FILTER] Rejecting overlapping collision at frame {frame_idx}: {b['name']} (distance={center_distance:.1f} < {min_separation:.1f})"
                    )
                    continue

                # Ball còn detect, va chạm ở frame N
                collisions.append(
                    {"frame_idx": frame_idx, "cueball": cueball, "ball": b}
                )
            else:
        ...
        if frame_idx in collisions_by_frame:
            ...
            for coll in frame_collisions:
                ...
                cv2.line(
                    ...
                    (0, 255, 255),
                    3,
                )  # Màu vàng đậm cho đường va chạm

                # Hiển thị thông tin va chạm
                cv2.putText(
                    frame_draw,
                    f"Collision with {ball['name']}",
                    (20, 40),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    1,
                    (0, 255, 255),
...
def log_collisions(collisions, log_path):
    ...
    with open(log_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["frame_idx", "cue_x", "cue_y", "ball_name", "ball_x", "ball_y"]
        )
        for c in collisions:
            writer.writerow(
                [
...
def get_collisions(
    ...
    return get_collisions_from_data(
        frames_data, cue_ball_name, move_thresh, contact_margin
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Detect collision from YOLO CSV output and video."
    )
    ...
    parser.add_argument(
        "--cue-ball-name",
        default="cueball",
        help="Cue ball class name (default: cueball)",
    )
    parser.add_argument(
        "--enable-visual", action="store_true", help="Enable collision visualization"
    )
    parser.add_argument(
        "--enable-log", action="store_true", help="Enable collision log (CSV)"
    )
    parser.add_argument(
        "--move-thresh",
        type=float,
        default=DEFAULT_MOVE_THRESH,
        help=f"Movement threshold as ratio of radius (default: {DEFAULT_MOVE_THRESH} = {DEFAULT_MOVE_THRESH*100:.0f}%%)",
    )
    parser.add_argument(
        "--contact-margin",
        type=float,
        default=DEFAULT_CONTACT_MARGIN,
        help=f"Extra pixels allowed for contact check (default: {DEFAULT_CONTACT_MARGIN})",
    )
    ...
    collisions_raw = get_collisions(
        csv_path, cue_ball_name, move_thresh, contact_margin
    )

    # Convert lại về format cũ cho visualize và log
    collisions = [
        {"cueball": c["cueball"], "ball": c["ball"], "frame_idx": c["frame_id"]}
